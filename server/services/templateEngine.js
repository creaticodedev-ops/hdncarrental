...
export const TEMPLATE_VARIABLES = [
  ...
  { key: 'identity_expires_on', label: 'ID Expiration Date', group: 'customer' },
  { key: 'identity_issued_on', label: 'ID Expiration Date (legacy key)', group: 'customer' },
...
export const buildTemplateVariables = (booking, { contractNumber, owner, agency = {}, template = {}, includeCompanyStamp = true } = {}) => {
  ...
  const values = {
    ...
    identity_expires_on: firstNonEmpty(mergedBooking, ['identityExpiresOn', 'identityExpiryDate']) || '—',
    // Legacy placeholder: prefer expiration so customized templates still print the CIN date.
    identity_issued_on:
      firstNonEmpty(mergedBooking, ['identityExpiresOn', 'identityExpiryDate', 'identityIssuedOn', 'identityIssueDate']) || '—',
  ...
  return {
    ...
    identityExpiresOn: values.identity_expires_on,
