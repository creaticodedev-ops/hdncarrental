...
export const relativeDayKey = (value) => {
  const ymd = agencyYmd(value)
  if (!ymd) return 'other'
  const today = agencyYmd(new Date())
...
export const formatRelativeDay = (value, language, t) => {
  const key = relativeDayKey(value)
  if (key === 'today') return t('admin.bookings.relativeToday')
  if (key === 'tomorrow') return t('admin.bookings.relativeTomorrow')
  if (key === 'yesterday') return t('admin.bookings.relativeYesterday')
  return formatDay(value, language)
}

export const formatRelativeWhen = (value, language, t) => {
  const time = formatTime(value, language)
  const day = formatRelativeDay(value, language, t)
  return time ? `${day} ${time}` : day
}

export const getOpsFlags = (booking) => {
  const today = agencyYmd(new Date())
  ...
  return {
    due: pay.remaining > 0 && !closed,
    unsigned: sig !== 'signed' && !closed,
    expired: sig === 'expired' && !closed,
    departingToday: agencyYmd(booking?.pickupDate) === today && !closed,
    returningToday: agencyYmd(booking?.returnDate) === today && status !== 'cancelled',
    onRent: status === 'active',
  }
}

/**
 * Single next move for the desk. Collect first (money is blocking),
 * then signature, then the natural status step.
 */
export const getNextDeskAction = (booking) => {
  if (!booking || booking.status === 'cancelled') return { id: 'open' }
  const flags = getOpsFlags(booking)
  const pay = bookingPaymentFigures(booking)
  const sig = getSignatureStatus(booking)

  if (flags.due) return { id: 'collect', amount: pay.remaining }
  if (flags.expired || flags.unsigned) return { id: 'sign' }
  if (booking.status === 'pending') return { id: 'confirm' }
  if (booking.status === 'confirmed') return { id: 'ready' }
  if (booking.status === 'ready_for_pickup') return { id: 'start' }
  if (booking.status === 'active' && flags.returningToday) return { id: 'complete' }
  if (booking.status === 'completed') return { id: 'open' }
  if (booking?.completion?.contractPdfUrl || sig === 'signed') return { id: 'view' }
  return { id: 'open' }
}

export const nextDeskActionLabel = (action, t, currency) => {
  if (!action) return t('admin.bookings.view')
  if (action.id === 'collect') {
    return t('admin.bookings.nextCollect', { amount: money(currency, action.amount) })
  }
  const map = {
    sign: 'admin.bookings.nextSign',
    confirm: 'admin.bookings.nextConfirm',
    ready: 'admin.bookings.nextReady',
    start: 'admin.bookings.nextStart',
    complete: 'admin.bookings.nextComplete',
    view: 'admin.bookings.viewContract',
    open: 'admin.bookings.view',
  }
  return t(map[action.id] || 'admin.bookings.view')
}

export const STATUS_RAIL = ['pending', 'confirmed', 'ready_for_pickup', 'active', 'completed']

export const nextStatusInRail = (status) => {
  const idx = STATUS_RAIL.indexOf(status)
  if (idx < 0 || idx >= STATUS_RAIL.length - 1) return null
  return STATUS_RAIL[idx + 1]
}

export const sortTodayDesk = (bookings) => {
  const today = agencyYmd(new Date())
  const rank = (booking) => {
    if (agencyYmd(booking.pickupDate) === today) return 0
    if (agencyYmd(booking.returnDate) === today) return 1
    return 2
  }
  const when = (booking) => {
    const ts = new Date(
      agencyYmd(booking.pickupDate) === today ? booking.pickupDate : booking.returnDate,
    ).getTime()
